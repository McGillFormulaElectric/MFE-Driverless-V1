...
namespace lidar_cone_detector
{
    ...
    class GroundPlaneRemovalNode ::GroundPlaneRemovalNode(const rclcpp::NodeOptions &options) : Node("ground_plane_removal_node", options)
    {
    public:
    ...
    private:
        ...
        void remove_ground_plane_callback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
        {
            ...
            // Voxel downsampling
            // TODO: use passthrough filter instead of voxel? Voxel might ruin the centroid
            ...
            if(coefficients->values.size() >= 4)
            {
                RCLCPP_INFO(
                    ...
                    coefficients->values[3]); // macro for logger
            } 
            else
            {
