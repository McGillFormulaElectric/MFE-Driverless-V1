...
# Collect test files
TESTS=(
    # Algorithm unit tests (always run, no bag needed)
    ...
    # Rosbag integration tests (skip automatically when bag is empty/missing)
    "$SRC/mfe_perception/test/test_rosbag_lidar.py"
    "$SRC/mfe_state_estimation/test/test_rosbag_ekf.py"
    "$SRC/mfe_path_planning/test/test_rosbag_path_planning.py"
)
